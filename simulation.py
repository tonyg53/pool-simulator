...
def main():
    ...
    with open('ballPos.csv', 'w+', newline='') as csvfile:
        ballwriter = csv.writer(csvfile)
        while ballsMoving :
            countStoppedBalls = 0
            for k, ball in enumerate(ballList): 
                ...
                if(k==0): ballData = [ball.Loc.x, ball.Loc.y]
                elif(k==1): ballData = [ball.Loc.x, '', ball.Loc.y]
                elif(k==2): ballData = [ball.Loc.x,'','',ball.Loc.y]
                elif(k==3): ballData = [ball.Loc.x,'','','',ball.Loc.y]
                ballwriter.writerow(ballData)
